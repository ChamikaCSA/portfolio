...
function ToggleGroup({
  ...
  return (
    <ToggleGroupPrimitive
      ...
      data-size={size}
      className={cn(
        'group/toggle-group flex w-fit items-center gap-0.5 rounded-2xl border border-line p-0.5',
        className,
      )}
      {...props}
    >
      <ToggleGroupProvider value={{ variant, size }}>
        {props.type === 'single' ? (
          <ToggleGroupHighlightPrimitive className="rounded-[0.9rem] bg-wash">
            {children}
...
function ToggleGroupItem({
  ...
  return (
    <ToggleGroupHighlightItemPrimitive
      value={props.value}
      className={cn(type === 'multiple' && 'rounded-[0.9rem] bg-wash')}
    >
      <ToggleGroupItemPrimitive
        ...
        className={cn(
          ...
          'min-w-0 flex-1 shrink-0 rounded-[0.9rem] border-0 shadow-none focus:z-10 focus-visible:z-10',
          className,
        )}
        {...props}
      >
