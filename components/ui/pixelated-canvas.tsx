...
type PixelatedCanvasProps = {
  ...
  fadeSpeed?: number;
  /** Skip samples darker than this luminance (0–255). Useful for knocking out a black studio backdrop. */
    ...
    const compute = () => {
      ...
      for (let y = 0; y < offscreen.height; y += cellSize) {
        ...
        for (let x = 0; x < offscreen.width; x += cellSize) {
          ...
          const drop =
            hash2D(cx, cy) < dropoutProb ||
            (luminanceKnockout > 0 && Lc < luminanceKnockout);
