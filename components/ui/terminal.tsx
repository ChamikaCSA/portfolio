...
export const Terminal = ({
  ...
  useEffect(() => {
    if (!sequence || !onComplete || completedRef.current) return
    if (childCount === 0 || activeIndex < childCount) return
    completedRef.current = true
    onComplete()
  }, [sequence, onComplete, activeIndex, childCount])
  ...
  const content = (
    <div
      ...
      className={cn(
        chrome &&
          "border-border bg-background z-0 h-full max-h-100 w-full max-w-lg rounded-2xl border",
        className
      )}
    >
      {chrome ? (
      ...
      <pre className={chrome ? "p-4" : undefined}>
